...
var Carousel = _react2['default'].createClass({
  ...
  propTypes: {
    ...
    monitorImagesLoaded: _react2['default'].PropTypes.bool
  },
  ...
  bindEvents: function bindEvents() {
    ...
    if (_exenv2['default'].canUseDOM) {
      ...
      if (this.props.monitorImagesLoaded) {
        var node = _reactDom2['default'].findDOMNode(self);

        if (node) {
          var imgLoad = imagesLoaded(node.querySelectorAll('*'), { background: true });
  ...
  getListStyles: function getListStyles() {
    ...
    var transform = 'translate(' + this.getTweeningValue('left') + 'px, ' + this.getTweeningValue('top') + 'px)';
    return {
      transform: transform,
